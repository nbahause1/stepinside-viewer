// Export ground-truth ROOM FACTS from the 3DGS scan for the grounded planner.
// Reuses analyze-scene.mjs's computation (same cached point cloud) but, instead
// of drawing the map, writes staging-refs/_out/room-facts.json:
//   { footprint:{width,depth}, floor_area, placeable_area,
//     wall_lengths:{left,right,near,far}, sofa_wall, openings_summary }
//   node --env-file=.dev.vars room-analysis/export-facts.mjs
...
// ---- 6. Facts -----------------------------------------------------------------
// Areas from the cleaned masks; wall spans from the room bounding box: the
// left/right long walls run along Z (depth), the near/far walls along X (width).
let roomA=0, placeA=0; for(let p=0;p<N;p++){roomA+=room[p];placeA+=place[p];}
const width = (rx1-rx0)*cellX, depth = (rz1-rz0)*cellZ;
const round2 = v => Math.round(v*100)/100;
const facts = {
  footprint: { width: round2(width), depth: round2(depth) },
  floor_area: round2(roomA*cellArea),
  placeable_area: round2(placeA*cellArea),
  wall_lengths: {
    left:  round2(wFarZ - wNearZ),   // long wall along Z
    right: round2(wFarZ - wNearZ),   // long wall along Z
    near:  round2(wRightX - wLeftX), // wall along X
    far:   round2(wRightX - wLeftX), // wall along X
  },
  sofa_wall: sofaSide,
  openings_summary: Object.fromEntries(
    Object.entries(walls).map(([name, w]) => [
      name,
      w.openings.length
        ? w.openings.map(o => ({ type: o.type, length: round2(o.len) }))
        : 'solid',
    ]),
  ),
};

const outJson = join(OUT_DIR, 'room-facts.json');
writeFileSync(outJson, JSON.stringify(facts, null, 2));

// ---- 7. Report ----------------------------------------------------------------
console.log('\n=== ROOM FACTS ===');
console.log(`footprint: ${facts.footprint.width} x ${facts.footprint.depth}  (scan units)`);
console.log(`floor area ${facts.floor_area}  placeable ${facts.placeable_area}`);
