...
export default function Demos() {
  ...
  useEffect(() => {
    if (!expanded) return;
    // Robust iOS scroll lock. `overflow: hidden` alone does NOT stop iOS Safari
    // from scrolling the page when the on-screen keyboard opens inside the
    // (nested) viewer iframe — the concierge's text input did exactly that,
    // sliding the page out from under the full-screen viewer so the site behind
    // peeked through and could even be scrolled. Pinning the body with
    // position:fixed at the current offset is the technique that actually holds.
