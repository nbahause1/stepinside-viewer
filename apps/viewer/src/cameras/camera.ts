...
class Camera {
    ...
    /**
     * Render-only gaze offset in degrees (x = pitch, y = yaw), layered on top
     * of `angles` at the entity for the walking head-scan. Kept SEPARATE from
     * `angles` on purpose: click-to-walk steering reads `angles` to compute its
     * heading error, so if the scan lived in `angles` the navigation would read
     * it back and steer to cancel it (heading = target − gaze ⇒ no visible
     * scan). As a separate render offset it scans around the travel heading and
     * can never be cancelled by the nav.
     */
    gazeYaw = 0;
    ...
    copy(source: Camera) {
        ...
        this.fov = source.fov;
        this.gazeYaw = source.gazeYaw;
        this.gazePitch = source.gazePitch;
    }
    ...
    lerp(a: Camera, b: Camera, t: number) {
        ...
        this.gazeYaw = math.lerp(a.gazeYaw, b.gazeYaw, t);
        this.gazePitch = math.lerp(a.gazePitch, b.gazePitch, t);
    }
