...
class NavCursor {
    ...
    constructor(
        ...
        this.onPointerMove = (e: PointerEvent) => {
            ...
            // Store the position and defer the (expensive) collision pick to the
            // next frame. pointermove fires many times per frame on high-rate
            // mice/trackpads; running a raycast + SVG rebuild per event janked
            // the cursor (it "froze" on fast moves). One pick per frame is smooth.
        ...
        // Hover pick runs at most once per frame (see onPointerMove). Done on
        // 'update' (fires every frame, unlike 'prerender' which only fires when
        // the scene actually renders) so the cursor still follows the pointer
        // while the camera is still.
        this.onUpdate = () => {
            if (this.hoverDirty && this.pointerInside) {
                this.hoverDirty = false;
                this.updateCursor(this.lastOffsetX, this.lastOffsetY);
            }
        };
